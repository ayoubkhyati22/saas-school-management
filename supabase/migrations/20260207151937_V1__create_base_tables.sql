...
-- Schools: Super admins can view all, school admins can view their own
...
-- Plan features: All authenticated users can view features of active plans
CREATE POLICY "All authenticated users can view plan features"
  ON plan_features FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM subscription_plans
      WHERE subscription_plans.id = plan_features.subscription_plan_id
      AND subscription_plans.active = true
    )
  );

-- Subscriptions: Super admins can view all, others can view their school's
